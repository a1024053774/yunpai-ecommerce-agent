...
- WP5 独立验收未执行；完成前不构成 M10-R 签署或生产放行。
...
- 发现并修复 2 个问题：
  1. 决策台切换店铺/期间后，旧经营建议残留（刷新决策台只刷事实、不清建议面板）→
     改为每次刷新决策台都重置建议面板为提示，切换店铺不再显示上一店铺的建议。
  2. 交期证据 `source_reference` 仅判非空，纯空白字符串可绕过 → 改为
     `length(trim(source_reference)) > 0`，新增反例
     `test_formal_gate_blocks_blank_source_transport_evidence`。
- 复核未发现漏洞的项：粒度守卫 fail-closed（大小写/Unicode/空白输入均在入账校验层
  拒绝，任何文本差异都会 fail-closed）；建议证据引用硬校验（引用不在 catalog、数值
  不匹配、格式非法均拒绝）；供货政策 SKU 绑定 + 时效。
- 已知残留（记录在案）：粒度守卫只比较“声明的粒度字符串”，不交叉校验 order_id/sku_id
  与声明是否一致（开发负责人已选择“暂时这么修”）；交期证据仍为店铺级（无 SKU 数据源）；
  时间字段用 ISO 字符串字典序比较，代码库统一 UTC 下正确，混时区导入时需注意。
