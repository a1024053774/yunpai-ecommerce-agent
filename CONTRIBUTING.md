...
定向测试示例（按改动选择文件或测试节点；本仓库固定屏蔽代理，否则模型网关的测试会挂）：

```bash
NO_PROXY=127.0.0.1,localhost no_proxy=127.0.0.1,localhost ALL_PROXY=http://127.0.0.1:9 HTTP_PROXY=http://127.0.0.1:9 HTTPS_PROXY=http://127.0.0.1:9 .venv/bin/python -m pytest tests/test_competitive.py -q
```

需要全量时，把测试路径替换为 `tests`，保留上述环境变量。测试范围与触发条件
统一见第 6 节；全量耗时以本次运行记录为准，不用历史耗时决定是否重跑。
仓库里的 `scripts/run_full_regression.py` 也是显式调用的收口工具，不是编辑后自动钩子。
...
## 测试
定向：`.venv/bin/python -m pytest tests/test_competitive.py -q`（填写实际结果与覆盖范围，环境变量见第 2 节）
全量：填写当前候选的 CI 运行链接或本地命令、日期、commit 与实际结果；待 CI 时标注待验证
未运行：填写检查项与原因，例如纯文档改动未运行本地 pytest
...
**开发期默认跑最小必要范围，不因每次编辑、提交或普通任务结束而自动跑全量。**
选择范围前先看受影响行为、直接调用方、共享契约和相关测试：

- **纯文档、注释或格式调整**：确认不改变运行行为后，检查文档内容、引用与
  `git diff --check`，无需在本地运行 pytest。
- **局部行为改动**：跑覆盖该行为及直接调用方的测试文件或测试节点；缺陷修复
  保留失败反例，修复后复验。
- **跨模块改动**：在定向测试之外，加跑直接相邻模块和共享接口的回归。
- **高风险改动**：数据库迁移、共享 schema/枚举/注册表、鉴权与数据隔离、
  语义路由和 stream/non-stream 一致性、共享 fixture 或测试基础设施，必须覆盖
  相关领域回归；影响范围无法可靠界定、失败暴露更广影响时，再扩大到全量。
  第 9 节迁移合并门禁、第 10 节真实模型基准及明确的专项验收要求仍须满足。

**全量集中在候选收口与集成门禁。** WP 收口、PR 合并前和发布前，为当前候选
保留一次全量通过结果；优先复用适用的 CI 结果。定向测试通过后可以先提 PR，
全量标注“待 CI”，合并门禁须等全量通过；CI 不可用或未覆盖所需范围时本地补跑。

CI 的触发事件和实际检查以 `.github/workflows/ci.yml` 为准；当前在 PR 和推送
到 `main` 时执行全量。同一候选、相同测试范围与相关环境已有通过结果时，不必
在本地重复。PR 通过不替代合并后组合代码的检查，合并后的全量可由 `main` CI
承担。待执行、失败或未覆盖当前代码/fixture/依赖/测试配置的旧结果不能算通过。

通过选定范围后，只有新改动、失败或尚未解决的影响范围疑点才触发扩大或重跑。
记录实际命令、范围、结果和未运行项的原因；引用 CI 时附运行链接与对应提交，
不能把“未运行”写成“通过”。
...
所以：**占号要提前说，迁移合并后必须验证合并结果的全量**，可使用该合并提交
的 `main` CI 通过结果，无需本地再跑一遍。这是迁移集成门禁，普通开发按第 6 节
分层执行。`grep` 到语句在文件里不等于它会被执行，同名函数、同名类方法、
同名字典键都可能被静默覆盖。
...
| **40** | M10-R / 1688 A1 | 渠道可售量持久化 | 独立 `channel_availability_snapshots` / `channel_availability_records`，保存 `amountOnSale` 的商品级/SKU 级事实与来源版本；不写入 WMS `inventory_balances` | **服务器 A1 候选已完成真实全量/增量/对账并通过；未合并，未升应用包版本** |
| **41** | M10-R / 1688 A1 P1 | 连接器同步 checkpoint | 独立 `connector_sync_checkpoints`：按 tenant/connector/store/resource + full/incremental 窗口身份持久化 cursor、watermark、租约与对账统计；不把店铺编码进 resource，不改写已执行的 `_apply_v40` | **本地候选已完成；服务器尚未执行 v41** |
| 42+ | *（空闲）* | | | |

旧 M5 工作包 3 对 v28 的预留已随路线冻结取消；M5-R 的 `_apply_v28` 已合入 `main`。
26–30、32、33、34、35 已在 `main`。**31 仍未进入 `main`**，`main` 的 `initialize()` 目前是 30→32→33→34→35，故意没有 `_apply_v31`。**35 由 M7-R WP3（Product Identity）占用，36 由 M9-R WP3（生命周期建议）占用，37 由 M10-R WP3（订购单）占用，38 由 M10-R WP4（费用底账）占用，39 由 M9-R WP5（商品事实 item 隔离修复）占用，40 由本分支 A1 渠道可售量持久化占用，41 由本分支 A1 P1 checkpoint/watermark 占用**，下一空闲号从 **42** 起。v41 只追加 `connector_sync_checkpoints`；灾备 manifest 仍精确匹配当前 schema，升级后必须立即生成并验证新的全量备份，v40 归档只能由匹配旧 schema 的程序在隔离环境恢复。
...
6. 合并后取得该合并提交的全量通过结果（`main` CI 或本地），再关掉这一行
