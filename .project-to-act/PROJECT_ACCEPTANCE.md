...
- 结论：2026-08-30 已将公网产品 Demo 的访问边界从 7 天 Cookie 门禁切换为 Nginx HTTP Basic Auth，未改变应用代码、schema v39 或包版本 0.30.0。认证范围覆盖 `/admin`、`/admin/advanced`、`/customer-test`、`/health`、`/ready`、知识图谱页面和 `/v1/*`；页面与产品 API 共用同一密码文件，应用仍只监听 `127.0.0.1:8767`，AgentLoop 独立域名保持原链路。
  ...
  本结论只覆盖公网认证切换和瞬时隔离验证，不扩大 virtual Demo 的真实渠道、生产数据、写权限、长稳、异机媒体灾备、正式 WP5 或 `G-PROD-001` 放行范围；共享密码不写入项目记录。

- 结论：2026-08-29 对已发布的统筹 Agent Demo 追加历史会话横向滚动修复。红态公网 `/admin` 的 `.chat-shell` 内部列被内容撑到约 4000px，`.chat-head` 与 `#conversationList` 没有有限宽度，`clientWidth == scrollWidth`，无法到达右侧会话；修复提交 `c3812313502493571f968c025969974893effd47` 已普通推送到 `origin/demo/main-multimodal-product-demo` 并部署到 `/opt/yunpai-ecommerce-agent-releases/c3812313502493571f968c025969974893effd47`。`tests/test_workspace_conversations.py` 先红后绿；相关回归 `79 passed in 33.23s`，compileall、`git diff --check` 和 managed 台账校验通过。公网 `/admin` 实测会话条 `clientWidth=612`、`scrollWidth=3960`、`overflow-x=auto`，实际 `scrollLeft=3348.5`，最右侧历史卡片可见；health/ready 200，部署回执与远端 HTML 均指向该提交。证据为 E-20260829-002；本条只验收 UI 修复，不重写 E-20260829-001 的整合历史，也不豁免真实平台、真实数据/写权限、媒体异机灾备、长稳、正式独立 WP5 或 G-PROD-001。
- 结论：2026-08-29 已将统筹 Agent 公网 Demo 收口到提交
  `845a0f52f728b931a5f8b739c421f6b24c9869a6`，并推送到
  `origin/demo/main-multimodal-product-demo`。公网 `/` 与 `/app` 均 302 到 `/admin`，不再进入
  AgentLoop；`/admin` 可直接 Ctrl/⌘+V 粘贴图片并在后续轮次使用已持久化、脱敏、
  `semantic_authority=false` 的观察，当前图片失败时不会借旧图。装载 virtual 数据后，统筹真实
  公网会话已逐项调用商品/多仓库存/订单、营销/利润、预测/库存计划、生命周期建议/审计和
  流量实验；`/admin/advanced` 在同一服务与数据库展示对应结构化事实，`/customer-test` 实测
  Qwen2.5-VL 与 Qwen3-14B Polish 均为 applied。流量 effect/可信区间按实验 ID 从持久事实读取，
  两份实验均为 `verified_final/completed`。最终全量 `1511 passed, 1 skipped`（381.64 秒），
  75 项 workspace/presenter/multimodal 回归、compileall、diff check、managed 台账校验和
  design-integrity scanner 均通过；三个页面 console error/warning 均为 0。服务 health/ready
  200，无 Cookie 的三个入口 403，发布前后备份均验证。证据为 E-20260829-001；本结论只放行
  受鉴权 virtual 产品测试，不豁免真实平台渠道/数据/写权限、完整媒体异机灾备、24/72 小时
  长稳、正式独立 WP5 或 G-PROD-001。
...
- 结论：2026-08-28 已修复用户在其他电脑看到的统一 `model_unavailable`，证据 ID 为
  ...
  `Insufficient Balance`，不是浏览器、Cookie、Nginx 或 4090 隧道故障。未增加规则回答；
  `ModelGateway` 新增 Qwen/vLLM `chat_template_kwargs.enable_thinking` 单一供应商映射，并将
  公网主模型切换为 4090 `qwen3.6-35b-a3b-fp8-gpu0-200k`。两条契约测试在旧代码分别以
  `KeyError` 红灯，修复后 `tests/test_llm.py` 26 项、相关模型/流式/API 44 项通过；独立
  design-integrity 复审 PASS，扫描器无风险标记。公网按截图
  原话复验库存和订单 SSE 均为 200、完整 `meta/delta/citations/result/done`、3 个来源且
  `model_fallback=false`；订单正确返回已发货但派送地址待核实。真实浏览器显示 35B 主模型，
  无兜底标签、console 0 error/warning。服务、4090 隧道、Nginx 和 certbot active；schema、
  数据契约、Agent 路由、Prompt 与 API 未变。DeepSeek 余额本身未充值，但已不在当前主链路；
  本结论仍不放行真实渠道、长稳、完整媒体灾备、M8-R WP5 或 `G-PROD-001`。
- 结论：2026-08-28 已按用户要求将 GitHub Demo 分支从
  `codex/main-multimodal-product-demo` 改名为 `demo/main-multimodal-product-demo`，
  证据 ID 为 E-20260828-004。操作前确认新名称不存在，先推送并核对新分支精确指向
  `4f3323abec18afb56ce181cf009e3c2938f89b1e`，再删除旧远端分支并 fetch/prune；最终新分支
  存在、旧分支不存在，本地只跟踪新分支。`origin/main`、服务器部署、代码、配置、测试与
  产品 Gate 均未改变，也未创建或合并 PR。
- 结论：2026-08-28 已把当前公网 Demo 对应实现提交
  `f647cfe204ea9dd842da7b1009bdc9426a77cf06` 推送到 GitHub 分支
  `codex/main-multimodal-product-demo`，证据 ID 为 E-20260828-003。提交父节点与当前
  `origin/main` 均为 `b77dfeb97583084214afc726ade367f3a7d99cea`；远端推送后
  `ls-remote` 与实现提交完全一致。提交前 37 个文件通过高置信度密钥、用户提供密码、私钥/
  `.env`、公网访问链接检查，`git diff --check` 与 project-to-act validate 均通过；服务器
  部署回执和本次代码/页面/测试文件 checksum dry-run 一致，只有部署后新增的本地台账更新
  不在服务器副本。没有重复运行全量测试，因为运行代码与 E-20260828-001 已通过的
  `1384 passed, 1 skipped` 完全相同；本轮只新增文档和 Git 元数据。GitHub `main` 未修改，
  未创建或合并 PR；产品 Demo 与生产 Gate 边界不变。
...
- 结论：2026-08-24 PR #19 已形成可推送的 M9-R 合成候选，证据 ID 为 E-20260824-002。
  ...
  固定 `v36/v39=M9-R`、`v37/v38=M10-R`、下一空闲号为 40。合成态迁移/item/读模型/
  灾备/发布关联 `64 passed`，WP1～WP4 四脚本、compileall、diff check 与台账校验均退出 0。
  本条证明上传候选已消除 PR 冲突并通过开发侧合成复验；最终远端 Head 必须是
  `e4d42bb` 的后代，且仍须由闫睿涵在该干净 Head 上执行正式 WP5，不能把本条写成独立签署。
...
- 结论：2026-08-23 M9-R 完成跨模型、AgentOps 多维交叉审查后的逐项修复与第二轮
  ...
  诊断、建议和持久化快照统一使用同一显式 listing revision，并接通 analysis-run 下钻；
  验收脚本假绿和已确认死代码已清除。M9-R 专项及直接接口 `253 passed`，WP1～WP4
  四脚本全部退出 0，M7 相邻回归 `94 passed`，compileall 与 whitespace 检查退出 0。
  AgentOps 第二轮按 T4 多租户生产系统口径、仅限 M9-R 直接能力复核，结论为
  `ready/high confidence`。该 ready 仅指 WP1～WP4 开发者自验收候选，不替代闫睿涵从
  干净状态执行并签署的独立 WP5，也不证明真实模型 benchmark、真实平台数据、长稳或
  生产发布 Gate 已完成。
- 结论：2026-08-22 M9-R 针对 AgentOps 复检遗留的审计归因、事务原子性和模型来源
  可追溯问题完成开发者修复与自验收，证据 ID 为 E-20260822-001。内容级幂等现在以实际
  返回的建议 ID 写审计；新建议创建、同 SKU 旧建议置 stale 及其审计位于同一数据库事务，
  任一写入失败会整体回滚；诊断与建议分别固化 Prompt 版本、模型供应方/名称和决策来源，
  且只记录非敏感来源元数据。AgentOps 末轮又发现通用审计晚于领域事务提交，故障注入
  `2 failed` 后将通用审计绑定同一连接事务，修复为 `2 passed`。全部 39 个 M9-R 测试
  文件分片得到 `236 passed`，其中真实
  浏览器门禁 `4 passed`；WP1～WP4 验收脚本均退出 0，compileall、whitespace 通过。WP1
  仍明确跳过竞品/退款域真实覆盖和 `material_code` 两项声明依赖；本结论只是开发者自验收
  候选，不替代闫睿涵从干净状态执行的独立 WP5，也不批准真实平台因果、长稳或生产放行。
...
- E-20260824-002：PR #19 上传候选合成复验。M9-R 修复对象 `bc5d316` 只含 18 个已核对
  ...
  main 完全一致。合成态关联 `64 passed in 41.78s`，WP1～WP4 四脚本、compileall、
  `git diff --check` 与 project-to-act validate 全部退出 0。证据有效范围是本地代码级上传
  候选，不代替远端回读、正式 WP5、真实模型、真实平台数据、长稳或生产放行。
...
|---|---|---|---|---|---|---|---|
| E-20260830-001 | 2026-08-30 | 先记录旧 Cookie 门禁匿名 403 与旧 access endpoint 的 302/Set-Cookie 红态；备份 Nginx 路由、Cookie map、密码文件和旧受限凭据；将产品页面、健康端点和 /v1/* 改为统一 Basic Auth，停用旧 map/access/logout 路径，运行 nginx -t、reload、回环与公网探针 | 候选认证烟测首次因密码目录 700 导致 Nginx 13 Permission denied；改为 710 且保留密码文件 640 root:www-data 后，服务器回环和本机公网匿名/认证探针、legacy 404、健康检查、systemd、端口隔离均退出 0 | 运行代码 c3812313502493571f968c025969974893effd47；schema v39；包 0.30.0；Nginx 配置 /etc/nginx/snippets/yunpai-product-demo-{basic-auth,locations}.conf；备份目录 /var/backups/yunpai-product-demo/basic-auth-20260830T012347Z | 匿名 /admin、/admin/advanced、/customer-test、/v1/modules、/health、/ready 均 401 且带 WWW-Authenticate；认证后均 200；/ 与 /app 302 到 /admin 且认证后最终 200；旧 access/logout 404 且不签 Cookie，旧 Cookie 不能绕过；公网 8767 未开放；共享密码与 hash 未记录 | 服务器 Nginx/systemd、服务器备份目录、本机公网 curl 探针、docs/handoff/unified-agent-multimodal-demo.md；密码不进入仓库、飞书、截图、台账或日志 | 认证文件、Nginx 路由、密码、应用端口或部署对象变化前有效；不替代 virtual Demo 的真实渠道/生产 Gate |
| E-20260829-002 | 2026-08-29 | 在公网 `/admin` 先复现历史会话横向条红态并记录 `.chat-shell`、`.chat-head`、`#conversationList` 的尺寸/滚动指标；新增 `tests/test_workspace_conversations.py::test_workspace_history_strip_is_constrained_for_horizontal_scroll`，旧代码先运行为失败，恢复修复后运行相关 workspace/presenter/multimodal 回归；执行 compileall、`git diff --check`、project-to-act validate；以不可变 release 目录切换 systemd/Nginx，并从公网有效 Cookie 会话实际横向滚动到最右卡片；回读已打开的飞书文档保存状态与评论数 | 红态 1 failed；修复后相关回归 `79 passed in 33.23s`；compileall、diff check、台账校验、部署、health/ready 和公网滚动探针退出 0；飞书显示“已经保存到云端”、评论 0，正文本轮追加 17 张图片 | 部署代码 `c3812313502493571f968c025969974893effd47`；`origin/demo/main-multimodal-product-demo`；schema v39；包 0.30.0；release `/opt/yunpai-ecommerce-agent-releases/c3812313502493571f968c025969974893effd47`；截图 `docs/evidence/20260829-c381231-admin-history-scroll-fixed.png` | 红态约 4000px 内容列且 `clientWidth == scrollWidth`；绿态 `clientWidth=612`、`scrollWidth=3960`、`overflow-x=auto`、`scrollLeft=3348.5`，最右侧卡片可见；systemd active，`/health` 与 `/ready` 200；远端 HTML 含修复 CSS；飞书正文追加 15 张选定证据图、1 张修复图和 1 张原始复现图，既有历史图片块未删除 | `docs/agent-workspace.html`、`tests/test_workspace_conversations.py`、`docs/handoff/unified-agent-multimodal-demo.md`、公网 `https://129.211.3.209:8800/admin?rev=c381231`、飞书文档 `https://ycnm7nn66q78.feishu.cn/wiki/HWecwI76biWY00kSN2tc5Wnunvb`；不保存访问凭据 | 该证据仅覆盖本次布局修复、瞬时公网状态和飞书正文附图回读；代码、CSS、会话历史结构、部署对象或相关证据变化前；不替代 E-20260829-001 的整合验收或生产 Gate |
| E-20260829-001 | 2026-08-29 | 在最终代码提交上运行全量 pytest、75 项 workspace/presenter/multimodal 回归、compileall、diff check、managed 台账校验与 design-integrity scanner；以不可变 release 目录发布到 systemd/Nginx 链路并验证切换前源码/数据备份与切换后数据备份；从公网有效 Cookie 会话逐项执行商品/多仓库存/订单、营销/利润、预测/库存计划、生命周期/审计、两份流量实验、统筹直接粘贴图片及跨轮观察、客服 Vision/Polish；核对 `/`、`/app`、无 Cookie 门禁、health/ready 和三个页面 console | 全量 pytest 退出 0：1511 passed, 1 skipped（381.64 秒）；75 项关联回归、compileall、diff check、scanner、服务、备份与公网 smoke 均退出 0；唯一 skip 为既有条件跳过 | 部署代码 `845a0f52f728b931a5f8b739c421f6b24c9869a6`；`origin/demo/main-multimodal-product-demo`；schema v39；包 0.30.0；release `/opt/yunpai-ecommerce-agent-releases/845a0f52f728b931a5f8b739c421f6b24c9869a6` | `/` 与 `/app` 均 302 `/admin`；无 Cookie 的 `/admin`、`/admin/advanced`、`/customer-test` 均 403，health/ready 200。统筹读取商品/库存/订单、营销/利润、预测/计划、生命周期/审计和流量均成功；两份流量实验为 `verified_final/completed` 且 tool success。直接粘贴图片为 `image/png`、Vision applied，后续无图轮继续回答且未调用经营工具；客服 Vision/Polish 均 applied；三个页面 console 0 error/warning | `docs/handoff/unified-agent-multimodal-demo.md`；`docs/evidence/20260829-845a0f5-*.png` 中列明的 15 张选定截图；服务器 `/opt/yunpai-ecommerce-agent/.deploy-revision`、release/backup 目录、`/etc/yunpai-product-demo/access-url`（只记录路径，不记录凭据）；公网 `https://129.211.3.209:8800/` | 代码、数据、模型、Nginx/systemd/隧道、访问门禁或截图对象变化前；公网/模型为 2026-08-29 瞬时证据。仅放行受鉴权 virtual Demo，不替代真实渠道/数据/写权限、完整媒体灾备、长稳、正式独立 WP5 或 G-PROD-001 |
