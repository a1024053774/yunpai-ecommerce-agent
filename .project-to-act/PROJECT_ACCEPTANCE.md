...
- 结论：2026-08-11 F-317 路由与对话容错补丁通过本机扩大专项候选，证据 ID 为 E-20260811-001。统筹模型继续自主选择工具，确定性代码未新增关键词到工具的硬路由；`workspace-router-v3` 将事实询问与执行动作区分写入规划契约，对未被安全层确认的动作计划再做一次模型复核。新增商品目录和近期订单两个管理员租户范围只读工具；工具参数被拒后可纠正重规划，同一无效调用不重复；最近对话进入最终整理；已有已核实事实时，后续规划或生成失败以产品语言安全降级，不再丢失整轮结果。真实 DeepSeek 10 类初测为 9/10，唯一失败是库存与订单均成功后第三次规划输出不合规；修复后同一纠错回归 1/1，保留 `degraded=planning_output_invalid`。退款与提示注入场景均只返回确认门禁、零写工具；证据账本完整性错误 0。扩大专项 74/74，Windows 增加条件依赖 `tzdata` 后原 Traffic Lab 时区失败点 1/1。全量 pytest 在 15 分钟边界超时且没有最终统计，因此不声明全量通过；真实平台、长稳、容量和生产放行仍阻塞。
...
- 结论：2026-08-08 完成提交 `20d3c427ed82620a883c362d1e6d568913271c1e`（包版本 0.30.0）的本机部署候选，证据 ID 为 E-20260808-005。Python 3.12 隔离环境与 editable 依赖安装成功；`init` 退出 0（schema v27、认证与 DeepSeek 配置有效）、离线 `eval` 20/20、虚拟店铺 `simulate-store` 18/18 且 `production_claim=false`、`model-probe` 成功（14 tokens，1188.4 ms）。原 8080 端口由既有 Docker/WSL 转发服务占用，未终止或覆盖，当前实例改为仅监听 `127.0.0.1:8081`；`/health`、`/ready`、`/admin`、`/openapi.json` 均返回 200。浏览器确认“云湃电商经营控制台”、主导航和管理员登录表单可见，console error/warning 为 0。该证据仅证明本机部署与虚拟/最小真实模型链路可用，不豁免真实淘宝/ERP、客户数据、长稳、容量、安全、灾备和最终生产放行 Gate。
...
|---|---|---|---|---|---|---|---|
| E-20260811-001 | 2026-08-11 | AAWO 真实 HTTP/SSE 10 类 DeepSeek 旅程、保留失败后的 correction profile r4；路由/展示/API/商品订单/运营/Traffic Lab 扩大 pytest；单独时区红绿；15 分钟全量 pytest；compileall、whitespace、project-to-act validate | 初测 1 个业务失败；修复后纠错与专项均 0；全量退出 124（超时、无终态） | base `5ea1744` + working tree；`workspace_agent=4df98237...`、`presenter=b11150d5...`、`tests=8b50a671...`、`pyproject=c5bd7244...`；schema v28；DeepSeek `deepseek-v4-flash` | 初测 9/10，库存+订单两个工具成功后第三次规划不合规；同例 r4 1/1，`degraded=planning_output_invalid`；退款/注入零写入且 confirmation=true；扩大专项 74/74，时区原失败点 1/1；证据账本 integrity 0；全量 15 分钟未完成，不宣称通过 | `%TEMP%/yunpai-workspace-routing-report-20260811.json`（SHA256 `473d6919...`）；`%TEMP%/yunpai-workspace-routing-evidence-20260811.sqlite`；`%TEMP%/yunpai-workspace-routing-correction-20260811.sqlite`；`src/ecommerce_agent/workspace_agent.py`；`tests/test_workspace_agent.py`；`pyproject.toml` | 路由 Prompt、工具目录、动作/参数门禁、SSE 降级、模型配置、Windows 时区依赖或相关测试变化前 |
...
|---|---|---|---|---|---|---|
| G-WORKSPACE-ROUTING-001 | 2026-08-11 | F-317 统筹路由与对话容错 | 事实/动作区分、模型动作复核、商品/订单列表、多轮指代、参数纠错、重复阻断、组合查询、提示注入、已有事实降级、Windows 时区 | 本机扩大专项通过；全量未完成 | E-20260811-001 | 仅限当前本机 DeepSeek、虚拟/本地数据和 74 项扩大专项；全量因 15 分钟超时无终态，不豁免真实平台、真实数据、长稳、容量、安全或生产 Gate |
