...
|---|---|---|---|---|---|---|---|
| E-20260810-001 | 2026-08-10 | 14 类合成序列、滚动回测、坏模型注入、稳定识别临时破坏/还原、定向与订单库存回归、compileall、whitespace、全量 pytest | 专项 0；全量 124 超时 | working tree；无 schema、依赖、API 或后台变化 | 合成 Gate 14/14；定向 7 passed；相关回归 13 passed；反证 12/14 failed 后还原；全量收集 626 项，16 分钟到 11% 超时，未记通过 | `src/ecommerce_agent/business/forecast_models.py`、`forecast_evaluation.py`、`evals/forecasting/`、`tests/test_demand_forecast_*.py` | 预测模型、场景、阈值、订单口径或数据质量契约变化前 |
...
|---|---|---|---|---|---|---|
| G-FORECAST-SYNTH-001 | 2026-08-10 | 销售需求预测合成评测 | 多类需求形态、滚动回测无泄漏、基线保护、场景级误差/偏差阈值、质量风险降级和反证 | 通过 | E-20260810-001 | 仅限确定性合成数据与无状态模型核心；全量回归超时，PR 保持 Draft；不豁免真实订单口径、库存/退货/促销数据接入、线上回测、持久化、采购审批或生产放行 |
