...
- E-20260810-006：F-312 销售需求预测模型与合成评测本机候选。9 个确定性无状态候选模型覆盖稳定、趋势、周季节、趋势叠加季节、间歇、块状、波动、零需求、冷启动和结构变化；促销日历缺失、日期缺失与缺货截断显式降级，不宣称可预测。14/14 合成场景通过场景级 WAPE、总量偏差、需求类型、基线保护与滚动回测无未来泄漏门禁；模型/评测定向 `7 passed`，订单/库存相关回归 `13 passed`，compileall 和 whitespace 通过。反证中临时移除稳定需求识别后，合成 Gate 由 14/14 降为 12/14 且测试如期失败，还原后复验通过；另有全零坏模型注入持续验证误差超限会使 Gate 失败。全量共收集 626 项，在 16 分钟上限时运行到 11% 后超时，未出现失败但不得记为通过。本证据仅限虚拟合成序列和无状态预测核心，不代表真实订单、促销、缺货、库存、补货或生产可用性。
...
|---|---|---|---|---|---|---|---|
| E-20260810-006 | 2026-08-10 | 14 类合成序列、滚动回测、坏模型注入、稳定识别临时破坏/还原、定向与订单库存回归、compileall、whitespace、全量 pytest | 专项 0；全量 124 超时 | working tree；无 schema、依赖、API 或后台变化 | 合成 Gate 14/14；定向 7 passed；相关回归 13 passed；反证 12/14 failed 后还原；全量收集 626 项，16 分钟到 11% 超时，未记通过 | `src/ecommerce_agent/business/forecast_models.py`、`forecast_evaluation.py`、`evals/forecasting/`、`tests/test_demand_forecast_*.py` | 预测模型、场景、阈值、订单口径或数据质量契约变化前 |
...
|---|---|---|---|---|---|---|
| G-FORECAST-SYNTH-001 | 2026-08-10 | 销售需求预测合成评测 | 多类需求形态、滚动回测无泄漏、基线保护、场景级误差/偏差阈值、质量风险降级和反证 | 通过 | E-20260810-006 | 仅限确定性合成数据与无状态模型核心；全量回归超时，PR 保持 Draft；不豁免真实订单口径、库存/退货/促销数据接入、线上回测、持久化、采购审批或生产放行 |
