...
class MessageMediaStore:
    def __init__(self, data_dir: Path):
        ...
        self._pending_storage_refs: dict[str, int] = {}
    ...
    def persist(self, message_id: str, image: ChatImageInput) -> dict[str, Any]:
        ...
        marker = self._pending_marker(target)
        temporary = self._temporary_path(target)
        with self._pending_lock:
            prior_owners = self._pending_storage_refs.get(storage_ref, 0)
            owner_registered = False
            published_here = False
            try:
                marker.touch(exist_ok=True)
                os.chmod(marker, 0o600)
                self._pending_storage_refs[storage_ref] = prior_owners + 1
                ...
                remaining = prior_owners
                if owner_registered and remaining > 0:
                    self._pending_storage_refs[storage_ref] = remaining
                elif owner_registered:
                    self._pending_storage_refs.pop(storage_ref, None)
                try:
                    temporary.unlink(missing_ok=True)
                    if remaining == 0:
                        if published_here:
    ...
    def mark_persisted(self, value: str | list[Any] | None) -> None:
        for item in parse_message_media(value):
            storage_ref = str(item["storage_ref"])
            with self._pending_lock:
                remaining = self._pending_storage_refs.get(storage_ref, 0) - 1
                if remaining > 0:
                    self._pending_storage_refs[storage_ref] = remaining
                else:
                    try:
                        self._temporary_path(self._resolve(storage_ref)).unlink(
                            missing_ok=True
                        )
                        self._pending_marker(self._resolve(storage_ref)).unlink(
                            missing_ok=True
                        )
                    except OSError:
                        # A referenced message now owns the file; reconciliation can
                        # remove the stale marker without risking the media itself.
                        pass
                    self._pending_storage_refs.pop(storage_ref, None)

    def mark_for_deletion(self, value: str | list[Any] | None) -> None:
        """Persist filesystem deletion ownership before database references vanish."""
        ...
        for item in parse_message_media(value):
            ...
            if not path.is_file():
                continue
            with self._pending_lock:
                marker = self._pending_marker(path)
                marker.touch(exist_ok=True)
                os.chmod(marker, 0o600)
        ...
        for item in parse_message_media(value):
            ...
            with self._pending_lock:
                ...
                if path.is_file():
                    path.unlink()
                    removed += 1
                self._temporary_path(path).unlink(missing_ok=True)
                self._pending_marker(path).unlink(missing_ok=True)
                try:
                    path.parent.rmdir()
                except OSError:
                    pass
    ...
    def remove_unreferenced(
        ...
        """Retry files whose explicit pending ownership survived a request."""
        ...
        markers = [
            path
            for path in self.root.rglob(f"*{_PENDING_SUFFIX}")
            if not path.is_symlink() and path.is_file()
        ]
        for marker in markers:
            path = marker.with_name(marker.name[: -len(_PENDING_SUFFIX)])
            storage_ref = path.relative_to(self.data_dir).as_posix()
            with self._pending_lock:
                if marker.is_symlink() or not marker.is_file():
                    continue
                if self._pending_storage_refs.get(storage_ref, 0) > 0:
                    continue
                try:
                    temporary = self._temporary_path(path)
                    if storage_ref in referenced_storage_refs:
                        temporary.unlink(missing_ok=True)
                        marker.unlink(missing_ok=True)
                        continue
                    if path.is_file():
    ...
    @staticmethod
    def _pending_marker(path: Path) -> Path:
        return path.with_name(f"{path.name}{_PENDING_SUFFIX}")
