...
_FACT_ATTRIBUTE_SUFFIX_RE = re.compile(
    r"(?:的)?(?:售价|价格|金额|容量|保修|库存|物流|运费|优惠|数量|时效|"
    r"重量|尺寸|颜色|型号|订单号|订单编号|售后|退款)(?:是|为)?$"
)
_FACT_DEMONSTRATIVE_PREFIX_RE = re.compile(r"^(?:这款|该款|这台|该台|这个|该)")
_LATIN_FACT_ATTRIBUTE_SUFFIX_RE = re.compile(
    r"\b(?:sale\s+price|price|cost|amount|capacity|warranty|stock|inventory|"
    r"shipping|freight|discount|quantity|weight|size|color|model|costs?|is|are|"
    r"was|were|sells?(?:\s+for)?|priced(?:\s+at)?)\b$",
    re.IGNORECASE,
)
_LATIN_FACT_DEMONSTRATIVE_PREFIX_RE = re.compile(
    r"^(?:(?:about|for|regarding)\s+)?(?:(?:the|this|that|these|those|a|an)\s+)+",
    re.IGNORECASE,
)
    ...
    def polish(
        ...
        *,
        ...
        if (
            ...
            or not self.settings.polish_enabled
        ):
...
def _restore_and_validate(candidate: str, protected: _ProtectedDraft) -> str:
    ...
    if candidate_numbers != original_numbers:
        raise PolishIntegrityError("numeric_tokens")
    if _fact_bindings(candidate) != _fact_bindings(original):
        raise PolishIntegrityError("fact_binding")
...
def _fact_bindings(text: str) -> Counter[tuple[str, str]]:
    """Bind protected values to the nearest preceding product/order identifier."""

    bindings: Counter[tuple[str, str]] = Counter()
    entities: set[str] = set()
    current_entity: str | None = None
    for clause in _FACT_CLAUSE_SPLIT_RE.split(text):
        values = list(_AUTO_PROTECT_RE.finditer(clause))
        if not values:
            continue
        prefix = clause[: values[0].start()]
        entity = _fact_entity(prefix)
        ...
        for value in values:
            bindings[(
                current_entity,
                _normalize_automatic_phrase(value.group(0)),
            )] += 1
    return bindings if len(entities) >= 2 else Counter()


def _fact_entity(prefix: str) -> str | None:
    identifiers = [
        match.group(0).lower()
        for match in _ALPHANUMERIC_TERM_RE.finditer(prefix)
    ...
    while entity and entity != previous:
        previous = entity
        entity = _FACT_ATTRIBUTE_SUFFIX_RE.sub("", entity).strip()
        entity = _LATIN_FACT_ATTRIBUTE_SUFFIX_RE.sub("", entity).strip()
    entity = _FACT_DEMONSTRATIVE_PREFIX_RE.sub("", entity)
    entity = _LATIN_FACT_DEMONSTRATIVE_PREFIX_RE.sub("", entity)
    entity = re.sub(r"\s+", " ", entity).strip(" \t\r\n:：-'\"“”‘’()（）")
    visible = re.sub(r"[^A-Za-z\u4e00-\u9fff]", "", entity)
    return entity.lower() if len(visible) >= 2 else None
